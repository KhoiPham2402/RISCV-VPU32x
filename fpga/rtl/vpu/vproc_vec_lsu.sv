...
module vproc_vec_lsu #(
    ...
    // For ST_LOAD, dmem_sync has 1-cycle read latency.  To keep the data
    // pipeline full, the VLSU issues the request for word N+1 while capturing
    // word N (the data for word N arrived from the prefetch issued the previous
    // cycle).  A separate prefetch fires during ST_IDLE so word 0 is ready the
    // moment ST_LOAD starts.
    //   req_ctr  — which word to REQUEST this cycle (one ahead during ST_LOAD)
    //   word_ctr_r — which word to CAPTURE this cycle (unchanged for stores)
    wire [31:0] req_ctr = (state_r == ST_LOAD) ? word_ctr_r + 32'd1 : word_ctr_r;

    // Byte address of the current word (word-aligned: bottom 2 bits always 0)
    wire [31:0] curr_byte_addr  = base_r + {req_ctr[29:0], 2'b00};
    ...
    // --- Memory bus ---
    // Prefetch: when ST_IDLE and a load is about to start, pre-read word 0 so
    // mem_ready (vlsu_ready) is asserted on the first ST_LOAD cycle.
    wire load_prefetch   = (state_r == ST_IDLE) && vls_valid && vls_is_load;
    // Stop issuing new reads once req_ctr reaches the last word index.
    wire load_req_active = (state_r == ST_LOAD)  && (req_ctr < num_words_r);

    assign mem_req   = load_req_active || (state_r == ST_STORE) || load_prefetch;
    assign mem_we    = (state_r == ST_STORE);
    // Prefetch uses rs1_data (base not yet latched to base_r); active load/store use curr_byte_addr.
    assign mem_addr  = load_prefetch ? rs1_data : curr_byte_addr;
