...
module uart #(
    ...
    localparam int DEFAULT_DIV = CLK_FREQ / (16 * BAUD_RATE) - 1;
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (!tx_busy && !tx_empty && ctrl_tx_en) begin
                // Load next byte: {stop=1, data, start=0}
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (!rx_active && ctrl_rx_en) begin
                ...
                if (!rx_sync1) begin
                    rx_active   <= 1'b1;
                    rx_baud_cnt <= {1'b0, baud_div[15:1]};  // half bit for start center
            ...
            // RX FIFO dequeue on register read (handled in TL-UL section)
        end
    end

    // ─────────────────────────────────────────────────────────────────────────
    // TL-UL slave register interface
    // Response is combinatorial (0-latency) for register reads/writes.
    // ─────────────────────────────────────────────────────────────────────────
    logic [7:0]  reg_off;
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (tl_read && reg_off == 8'h0C && !rx_empty)
